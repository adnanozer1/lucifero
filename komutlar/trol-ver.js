...
.setTitle('Bir hata oldu!').setDescription(`Üyelerin üzerinden verilecek bir **ROL ID** belirtmedin! 🤔

**Örnek:**
\`\`\`${Prefix}Trol-ver ${message.guild.roles.cache.random().id}\`\`\``).setColor('#9c5cb2'));
if(!message.guild.roles.cache.gt(args[0])) return message.channel.send(new Discord.MessageEmbed()
.setTitle('Bir hata oldu!').setDescription(`Üyelerin üzerinden verilecek bir **ROL ID** belirtmedin! 🤔
...
a.roles.addRole(message.guild.roles.cache.get(args[0]).id);
}
}, 2000)
})
};
